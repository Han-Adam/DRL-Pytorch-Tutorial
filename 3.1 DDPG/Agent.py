...
class DDPG:
    def __init__(
        ...
        # Network
        self.actor = Actor(s_dim, s_dim*5, a_dim, bound).to(device)
        self.actor_target = Actor(s_dim, s_dim*5, a_dim, bound).to(device)
        self.actor_target.load_state_dict(self.actor.state_dict())
        self.opt_actor = torch.optim.Adam(self.actor.parameters(), lr=lr_actor)
        self.critic = Critic(s_dim, a_dim, s_dim * 5).to(device)
        self.critic_target = Critic(s_dim, a_dim, s_dim * 5).to(device)
    ...
    def get_action(self, s):
        ...
        a = self.actor(s).detach().item()
        #  add randomness to action selection for exploration
        a = np.clip(np.random.normal(a, self.var), -self.bound, self.bound)
        return a
