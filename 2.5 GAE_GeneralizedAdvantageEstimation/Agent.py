...
class ActorCritic:
    ...
    def calculate_GAE(self, s, r, s_, done):
        with torch.no_grad():
            v = self.critic(s).squeeze()
            v_ = torch.cat((v[1:], self.critic(s_)))
            ...
            for t in range(length - 1, -1, -1):
                running_add = running_add * self.gamma * self.lambda_ * \
                              (1 - done[t]) + delta[t]
                GAE[t] = running_add
            return GAE

    def calculate_return(self, r, done):
        length = r.shape[0]
    ...
    def learn(self):
        ...
        GAE = self.calculate_GAE(s, r, s_[-1], done)
        discounted_r = self.calculate_return(r, done)
