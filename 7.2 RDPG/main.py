...
BATCH_SIZE = 64                                 # the number of samples for each train
MAX_TRAJECTORY_LEN = 16                          # maximum trajectory length
RNN_NUM_LAYERS = 1                              # number of LSTM layers, too deep, feature vanish
...
# MAX_EP_STEPS should be divided by MAX_TRAJECTORY_LEN
START_LEARN_STEP = 2049                         # the step for agent start to learn
RENDER = False                                  # whether render


agent = RDPG(s_dim=S_DIM,
             a_dim=A_DIM,
             hidden=HIDDEN,
             capacity=CAPACITY,
...
for episode in range(MAX_EPISODE):
    ...
    for ep_step in range(MAX_EP_STEPS):
        ...
        total_steps += 1
    # show record
    print('Episode:', episode, ' Reward: %i' % int(ep_r), agent.var)
    # if ep_r > -300:
    #     RENDER = True
