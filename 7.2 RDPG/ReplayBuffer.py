...
class ReplayBuffer:
    def __init__(self, s_dim, a_dim, capacity, max_trajectory_len, batch_size):
        self.capacity = capacity
        self.max_trajectory_len = max_trajectory_len
        self.s = torch.zeros(size=[capacity, max_trajectory_len, s_dim], dtype=torch.float, requires_grad=False)
        self.a = torch.zeros(size=[capacity, max_trajectory_len, a_dim], dtype=torch.float, requires_grad=False)
        self.s_ = torch.zeros(size=[capacity, max_trajectory_len, s_dim], dtype=torch.float, requires_grad=False)
        self.r = torch.zeros(size=[capacity, max_trajectory_len, 1], dtype=torch.float, requires_grad=False)
        self.done = torch.zeros(size=[capacity, max_trajectory_len, 1], dtype=torch.float, requires_grad=False)
    ...
    def store_transition(self, s, a, s_, r, done):
        ...
        self.a[trajectory_counter, self.step_counter] = torch.tensor(a, dtype=torch.float)
